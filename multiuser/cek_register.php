...
if (isset($_POST['kirim'])) {
    ...
    if (empty($users) || empty($pass)) {
        $notifikasi = "Data Anda Belum Terdaftar.";
        $error = true;
    } else {
        ...
        $query = "SELECT * FROM users WHERE username = '$users' AND password = '$pass'";
...
<body>
    <form action="" method="POST">
        <div class="min-h-screen flex flex-col items-center justify-center bg-gray-100">
            <div class="flex flex-col bg-white shadow-md px-4 sm:px-6 md:px-8 lg:px-10 py-8 rounded-3xl w-50 max-w-md">
                ...
                <div class="mt-4 self-center text-xl sm:text-sm text-gray-800">Masukan Email Dan Password Anda</div>
                <div class="mt-10">
                    ...
                    <div class="flex w-full">
                        <button name="kirim" type="submit" class="flex mt-2 items-center justify-center focus:outline-none text-white text-sm sm:text-base bg-red-500 hover:bg-red-600 rounded-2xl py-2 w-full transition duration-150 ease-in">
            ...
            <div class="flex justify-center items-center mt-6">
                <a href="#" target="_blank" class="inline-flex items-center text-gray-700 font-medium text-xs text-center">
                    <span class="ml-2">Belum Mempunyai Akun?
                        <a href="register.php" class="text-xs ml-2 text-blue-500 font-semibold">Daftar Disinii</a>
                    </span>
                </a>
            </div>
        </div>
    ...
    <div class="fixed inset-0 flex items-center justify-center bg-gray-100 z-50">
        <div class="rounded-lg bg-gray-50 px-16 py-14">
            ...
            <button onclick="location.href='<?php echo $error ? 'register_verifikasi.php' : 'nasabah.php'; ?>'" class="mx-auto mt-10 block rounded-xl border-4 border-transparent bg-orange-400 px-6 py-3 text-center text-base font-medium text-orange-100 outline-8 hover:outline hover:duration-300">
                <?php echo $error ? 'Coba Lagi' : 'Lanjutkan Pengajuan'; ?>
            </button>
        </div>
    </div>
    <script>
